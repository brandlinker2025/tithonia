...
const schema=z.object({customer:z.object({name:z.string().min(2).max(120),phone:z.string().min(10).max(20),email:z.union([z.string().email(),z.literal("")]).optional(),address:z.string().min(5).max(500),city:z.string().min(2).max(120)}),payment_method:z.enum(["cod","online"]),discount_code:z.string().max(40).optional(),items:z.array(z.object({product_id:z.string().uuid(),qty:z.number().int().min(1).max(20),size:z.string().max(30).optional(),color:z.string().max(40).optional()})).min(1).max(30)});
export async function POST(req:Request){
 ...
 const r=await supabaseRest("rpc/place_order",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({p_order_no:orderNo,p_customer:parsed.data.customer,p_payment_method:parsed.data.payment_method,p_items:parsed.data.items,p_discount_code:parsed.data.discount_code?.trim()||null})});
